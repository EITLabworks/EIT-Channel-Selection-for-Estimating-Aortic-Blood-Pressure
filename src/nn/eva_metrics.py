...
class EvaMetrics:
        ...
        for t in ["Test", "Vali"]:
            ...
            for p in ["Curve","Recon", "Paras"]:
                ...
                if q in self.metrics:
                    ...
                    l.update({"NMSE": self.metrics[q]["Standard"]["NMSE"]})
                    try:
                        if p == "Curve":
    ...
    # -------------------------------------- #
    def save_metrics(self, para=None):
        """
    ...
    # -------------------------------------- #
    def calc_nmse(self, y_true, y_predict):
        """
        Calculates the NMSE (of 1D or 2D data)
        # info NMSE can be defined as mse/mean(y_true**2) or mse/mean((y_true-y_true_mean)**2))mse/var(y)
        # here the decision is on the var(y) denominator
        """
        if self.bNMSE:
            print(f"NMSE {np.var(y_true)}")
            d2 = mean_absolute_error(y_true, y_predict) / (np.var(y_true))
            return 10*np.log10(d2)
        

    # -------------------------------------- #
    def calc_metrics(self, y_true, y_predict, paratype, name, bParas=False, bSave=True, bShow=False, pnames=[None]):
        """
        ...
        if self.byMinMax and bParas==False:
            ...
            d.update({"ByMinMax": s})
            if self.byMeanForm:
                self.visu_mean_results( mean_true, mean_predict,min_true, min_predict, max_true, max_predict, name, d, bSave, bShow=bShow, pnames=pnames)
    ...
    def visu_metrics_paper(self, ymean_true, ymean_predict, min_true, min_predict, max_true, max_predict, name, bSave,pigs=None, bShow=False):
        """
    ...
    def visu_mean_results(self, ymean_true, ymean_predict, min_true, min_predict, max_true, max_predict, name, d, bSave, pnames=[None],bShow=False):
        fontSize = 22
        title_Size = 24
